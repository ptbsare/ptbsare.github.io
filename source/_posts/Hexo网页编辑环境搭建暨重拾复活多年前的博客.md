...
&emsp;在平板上给予Chrome创建桌面图标的权限，使用Chrome首次打开`https://你的域名:端口`（请注意PWA貌似https是必要条件，有待验证）你会发现Code-Server和Web编辑器都是PWA可以一键安装到桌面，这样的好处是一点直接如同原生APP一样全屏进入无显示浏览器其他栏，沉浸式体验，写作体验丝滑；因为是网页APP，所以无论你身处何地，写完使用插件的CommitPush命令即可一键发布到Github自动构建更新博客，非常流畅。最后放几张小米平板和移动端手机的截图。
...
![IMG_20240618_110857.jpg](Hexo网页编辑环境搭建暨重拾复活多年前的博客/IMG_20240618_110857.jpg)